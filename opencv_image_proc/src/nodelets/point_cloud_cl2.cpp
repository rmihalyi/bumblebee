...
class PointCloudCl2Nodelet : public nodelet::Nodelet
{
...
void PointCloudCl2Nodelet::onInit()
{
  ...
  if (approx)
  {
    ...
    approximate_sync_->registerCallback(boost::bind(&PointCloudCl2Nodelet::imageCb,
                                                    this, _1, _2, _3, _4));
  }
  else
  {
    exact_sync_.reset( new ExactSync(ExactPolicy(queue_size),
                                     sub_l_image_, sub_l_info_,
                                     sub_r_info_, sub_disparity_) );
    exact_sync_->registerCallback(boost::bind(&PointCloudCl2Nodelet::imageCb,
                                              this, _1, _2, _3, _4));
  }

  // Monitor whether anyone is subscribed to the output
  ros::SubscriberStatusCallback connect_cb = boost::bind(&PointCloudCl2Nodelet::connectCb, this);
  ...
  pub_points2_  = nh.advertise<PointCloud2>("points_clahe2",  1, connect_cb, connect_cb);
...
void PointCloudCl2Nodelet::connectCb()
{
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  ...
  else if (!sub_l_image_.getSubscriber())
  {
    ...
    sub_disparity_.subscribe(nh,   "disparity_clahe", 1);
  }
}
...
void PointCloudCl2Nodelet::imageCb(const ImageConstPtr& l_image_msg,
                                 const CameraInfoConstPtr& l_info_msg,
                                 const CameraInfoConstPtr& r_info_msg,
                                 const DisparityImageConstPtr& disp_msg)
{
...
//    writer.write <pcl::PointXYZRGB> (ss.str(), cloud_, false);
    ...
    //tf_broad.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.,-M_PI/2.,0), tf::Vector3(0,0,0)), points_msg->header.stamp, "corrected", points_msg->header.frame_id));
...
PLUGINLIB_DECLARE_CLASS(opencv_image_proc, point_cloud_cl2,
                        opencv_image_proc::PointCloudCl2Nodelet, nodelet::Nodelet)
