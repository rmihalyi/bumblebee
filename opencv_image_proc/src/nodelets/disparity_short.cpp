...
#include <opencv_image_proc/DisparityShortConfig.h>
...
class DisparityShortNodelet : public nodelet::Nodelet
{
  ...
  typedef opencv_image_proc::DisparityShortConfig Config;
...
void DisparityShortNodelet::onInit()
{
  ...
  if (approx)
  {
    ...
    approximate_sync_->registerCallback(boost::bind(&DisparityShortNodelet::imageCb,
                                                    this, _1, _2, _3, _4));
  }
  else
  {
    exact_sync_.reset( new ExactSync(ExactPolicy(queue_size),
                                     sub_l_image_, sub_l_info_,
                                     sub_r_image_, sub_r_info_) );
    exact_sync_->registerCallback(boost::bind(&DisparityShortNodelet::imageCb,
                                              this, _1, _2, _3, _4));
  }

  // Set up dynamic reconfiguration
  ReconfigureServer::CallbackType f = boost::bind(&DisparityShortNodelet::configCb,
                                                  this, _1, _2);
  reconfigure_server_.reset(new ReconfigureServer(config_mutex_, private_nh));
  reconfigure_server_->setCallback(f);

  // Monitor whether anyone is subscribed to the output
  ros::SubscriberStatusCallback connect_cb = boost::bind(&DisparityShortNodelet::connectCb, this);
  // Make sure we don't enter connectCb() between advertising and assigning to pub_disparity_
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  pub_disparity_ = nh.advertise<DisparityImage>("disparity_short", 1, connect_cb, connect_cb);
}

// Handles (un)subscribing when clients (un)subscribe
void DisparityShortNodelet::connectCb()
{
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  ...
  else if (!sub_l_image_.getSubscriber())
  {
    ...
    sub_r_image_.subscribe(*it_, "center/image_rect", 1, hints);
    sub_r_info_ .subscribe(nh,   "center/camera_info", 1);
  }
}

void DisparityShortNodelet::imageCb(const ImageConstPtr& l_image_msg,
...
void DisparityShortNodelet::configCb(Config &config, uint32_t level)
{
...
PLUGINLIB_DECLARE_CLASS(opencv_image_proc, disparity_short,
                        opencv_image_proc::DisparityShortNodelet, nodelet::Nodelet)
