...
tf::TransformBroadcaster tf_broad_s;
class PointCloudS2Nodelet : public nodelet::Nodelet
{
  boost::shared_ptr<image_transport::ImageTransport> it_;

  // Subscriptions
  image_transport::SubscriberFilter sub_l_image_;
  message_filters::Subscriber<CameraInfo> sub_l_info_, sub_c_info_;
  ...
  //Simple incremental variable for pcd files
  bool initTime; 
  ros::Time timeFirstMsg;	
};

void PointCloudS2Nodelet::onInit()
{
  ...
  if (approx)
  {
    approximate_sync_.reset( new ApproximateSync(ApproximatePolicy(queue_size),
                                                 ...
                                                 sub_c_info_, sub_disparity_) );
    approximate_sync_->registerCallback(boost::bind(&PointCloudS2Nodelet::imageCb,
                                                    this, _1, _2, _3, _4));
  }
  else
  {
    exact_sync_.reset( new ExactSync(ExactPolicy(queue_size),
                                     sub_l_image_, sub_l_info_,
                                     sub_c_info_, sub_disparity_) );
    exact_sync_->registerCallback(boost::bind(&PointCloudS2Nodelet::imageCb,
                                              this, _1, _2, _3, _4));
  }

  // Monitor whether anyone is subscribed to the output
  ros::SubscriberStatusCallback connect_cb = boost::bind(&PointCloudS2Nodelet::connectCb, this);
  // Make sure we don't enter connectCb() between advertising and assigning to pub_points2_
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  //pub_points2_  = nh.advertise<pcl::PointCloud<pcl::PointXYZRGB> > ("point_cloud2",  1, connect_cb, connect_cb);
  pub_points2_  = nh.advertise<PointCloud2>("points_short",  1, connect_cb, connect_cb);
...
void PointCloudS2Nodelet::connectCb()
{
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  ...
  else if (!sub_l_image_.getSubscriber())
  {
    ...
    sub_c_info_   .subscribe(nh,   "center/camera_info", 1);
    sub_disparity_.subscribe(nh,   "disparity_short", 1);
  }
}
...
void PointCloudS2Nodelet::imageCb(const ImageConstPtr& l_image_msg,
                                 const CameraInfoConstPtr& l_info_msg,
                                 const CameraInfoConstPtr& r_info_msg,
                                 const DisparityImageConstPtr& disp_msg)
{
...
//	if(initTime == false)
//	{
//	timeFirstMsg = points_msg->header.stamp;
//	initTime = true;
//	}
	
//	pcl::fromROSMsg(*points_msg, cloud_);
//    std::stringstream ss;
//    ss << "output_points_short_" << (points_msg->header.stamp - timeFirstMsg).sec <<"-"<<(points_msg->header.stamp - timeFirstMsg).nsec <<".pcd";
//    writer.write <pcl::PointXYZRGB> (ss.str(), cloud_, false);
   
    pub_points2_.publish(points_msg);
    tf_broad_s.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(-0.001333558742668687,-0.0034659077605182966,-0.0009722837163996345, 0.8652978403416942), tf::Vector3(-0.0203424,0.000322832,-0.00499959)), points_msg->header.stamp, "short", "wide"));
...
PLUGINLIB_DECLARE_CLASS(opencv_image_proc, point_cloud_s2,
                        opencv_image_proc::PointCloudS2Nodelet, nodelet::Nodelet)
