...
class PointCloudW2Nodelet : public nodelet::Nodelet
{
...
void PointCloudW2Nodelet::onInit()
{
  ...
  if (approx)
  {
    ...
    approximate_sync_->registerCallback(boost::bind(&PointCloudW2Nodelet::imageCb,
                                                    this, _1, _2, _3, _4));
  }
  else
  {
    exact_sync_.reset( new ExactSync(ExactPolicy(queue_size),
                                     sub_l_image_, sub_l_info_,
                                     sub_c_info_, sub_disparity_) );
    exact_sync_->registerCallback(boost::bind(&PointCloudW2Nodelet::imageCb,
                                              this, _1, _2, _3, _4));
  }

  // Monitor whether anyone is subscribed to the output
  ros::SubscriberStatusCallback connect_cb = boost::bind(&PointCloudW2Nodelet::connectCb, this);
  ...
  pub_points2_  = nh.advertise<PointCloud2>("points_wide",  1, connect_cb, connect_cb);
...
void PointCloudW2Nodelet::connectCb()
{
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  ...
  else if (!sub_l_image_.getSubscriber())
  {
    ...
    sub_l_image_  .subscribe(*it_, "left_wide/image_rect_color", 1, hints);
    sub_l_info_   .subscribe(nh,   "left_wide/camera_info", 1);
    sub_c_info_   .subscribe(nh,   "right/camera_info", 1);
    sub_disparity_.subscribe(nh,   "disparity_wide", 1);
  }
}
...
void PointCloudW2Nodelet::imageCb(const ImageConstPtr& l_image_msg,
                                 const CameraInfoConstPtr& l_info_msg,
                                 const CameraInfoConstPtr& r_info_msg,
                                 const DisparityImageConstPtr& disp_msg)
{
...
//    writer.write <pcl::PointXYZRGB> (ss.str(), cloud_, false);
    ...
    tf_broad.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0,0,0), tf::Vector3(0,0,0)), points_msg->header.stamp, "wide", points_msg->header.frame_id));
...
PLUGINLIB_DECLARE_CLASS(opencv_image_proc, point_cloud_w2,
                        opencv_image_proc::PointCloudW2Nodelet, nodelet::Nodelet)
