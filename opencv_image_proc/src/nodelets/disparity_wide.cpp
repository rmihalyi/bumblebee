...
#include <opencv_image_proc/DisparityWideConfig.h>
...
class DisparityWideNodelet : public nodelet::Nodelet
{
  ...
  typedef opencv_image_proc::DisparityWideConfig Config;
...
void DisparityWideNodelet::onInit()
{
  ...
  if (approx)
  {
    ...
    approximate_sync_->registerCallback(boost::bind(&DisparityWideNodelet::imageCb,
                                                    this, _1, _2, _3, _4));
  }
  else
  {
    exact_sync_.reset( new ExactSync(ExactPolicy(queue_size),
                                     sub_l_image_, sub_l_info_,
                                     sub_r_image_, sub_r_info_) );
    exact_sync_->registerCallback(boost::bind(&DisparityWideNodelet::imageCb,
                                              this, _1, _2, _3, _4));
  }

  // Set up dynamic reconfiguration
  ReconfigureServer::CallbackType f = boost::bind(&DisparityWideNodelet::configCb,
                                                  this, _1, _2);
  reconfigure_server_.reset(new ReconfigureServer(config_mutex_, private_nh));
  reconfigure_server_->setCallback(f);

  // Monitor whether anyone is subscribed to the output
  ros::SubscriberStatusCallback connect_cb = boost::bind(&DisparityWideNodelet::connectCb, this);
  // Make sure we don't enter connectCb() between advertising and assigning to pub_disparity_
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
  pub_disparity_ = nh.advertise<DisparityImage>("disparity_wide", 1, connect_cb, connect_cb);
}
...
void DisparityWideNodelet::configCb(Config &config, uint32_t level)
{
...
PLUGINLIB_DECLARE_CLASS(opencv_image_proc, disparity_wide,
                        opencv_image_proc::DisparityWideNodelet, nodelet::Nodelet)
